...
const statistics = [
    {title: "43%", description: "Компаний в РФ уже сегодня успешно интегрировали AI агентов в свои бизнес-процессы"},
    {title: "2x", description: "Так автоматизация ускоряет бизнес-процессы компании, освобождая время для стратегии"},
    {title: "9 Месяцев", description: "Средний срок окупаемости  ИИ-проектов по автоматизации бизнес-процессов"},
]

const About = ({ abouts }: { abouts: AboutT[] }) => {
    const [open, setOpen] = useState<number | null>(null);

    return (
        <section className={"bg-white rounded-4xl rounded-b-none py-16 lg:rounded-[80px] lg:rounded-b-none lg:pt-26 lg:pb-26"}>
            <div className={'container mx-auto flex flex-col justify-between items-center gap-6 mb-18 md:mb-32 md:flex-row md:gap-10'}>
                ...
                <div className={'text-center md:text-start max-w-[860px]'}>
                    <p className={'sectionSubtitle'}>О нас</p>
                    <h2 className={'sectionTitle'}>Повышаем эффективность бизнеса при помощи ИИ</h2>
                    <h3 className={'sectionDescription max-w-[600px]'}>Помогаем компаниям внедрять ИИ и автоматизировать процессы, чтобы работать быстрее и эффективнее</h3>
                            ...
                            <div key={item.id ?? i}>
                                ...
                                <AnimatePresence initial={false}>
                                    {open === i && item.text && (
                                        ...
                                        >
                                            <div className="pt-2 pb-3 text-sm md:text-base text-[var(--color-gray)]">
                                                {item.text}
                                            </div>
                                        </motion.div>
                                    )}
            ...
            <div className={'mx-auto flex flex-col justify-center items-center px-4 gap-4 md:flex-row md:gap-5'}>
